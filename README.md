...
Move class: Keeps information about the player's movement and in the 
...
In the main program function, DFS and minimax are called to create and update the game tree. Then, a char variable is initialized at the root of the tree and will be used below for the current state of the game and a board variable that stores the table.
On this table, the changes that arise as a result of each player's movement will be stored.
Finally, within a while-loop, the game is implemented.
The computer's movement is to move from the current state to the next, using the nextMove field. Then, using the board field of the nextMove node,
we update the board. In turn, the player enters the elements of his movement, based on which the board changes,
and by calling findcur, we inform the node to which the computer has been transferred about its next move. This process is repeated until the game ends.
